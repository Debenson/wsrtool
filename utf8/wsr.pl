...
# fixed RPC::XML bug: https://rt.cpan.org/Public/Bug/Display.html?id=34472
binmode(STDIN, ':encoding(utf8)');
binmode(STDOUT, ':encoding(utf8)');
binmode(STDERR, ':encoding(utf8)');

# configer module Confluence 
Confluence::setRaiseError(1);
Confluence::setPrintError(1);

# global variables
my $gbl_url;         # Confluence wiki rpc service url
my $gbl_space_key;   # space key of Confluence wiki
my $gbl_parent_page; # page id of the parent page 
my $gbl_page_title;  # title of the destination
my $gbl_usr;         # user name
my $gbl_pwd;         # password
my $gbl_cfg;         # handle of program configuration
...
sub get_cfg_scalar_val {
	my ($cfg, $key, $is_pwd) = @_;
	if ($cfg->param($key) and ref($cfg->param($key)) ne 'ARRAY') {
		return $is_pwd? decode_base64($cfg->param($key)) : decode('utf-8', $cfg->param($key));
	} else {
		return undef;
	}
}
...
sub init_gbl_val {
	...
	# init. global variables.
	$gbl_url = get_cfg_scalar_val($gbl_cfg, 'RPC_XML_URI') unless $gbl_url;
	$gbl_space_key = get_cfg_scalar_val($gbl_cfg, 'SPACE_KEY') unless $gbl_space_key;
	$gbl_parent_page = get_cfg_scalar_val($gbl_cfg, 'PARENT_PAGE') unless $gbl_parent_page;
	$gbl_usr = get_cfg_scalar_val($gbl_cfg, 'login.usr') unless $gbl_usr;
	$gbl_pwd = get_cfg_scalar_val($gbl_cfg, 'login.pwd', 1) unless $gbl_pwd;	
}
...
# get weeks periods within a month or cross two months 
...
sub guess_period {
	my $att = shift;	
	my $period = '';
	if( $att =~ /(\d{4}|\d{2})\S*(\d{2})\S*(\d{2})[^\d]+(\d{4}|\d{2})\S*(\d{2})\S*(\d{2})/ ) {		
		my $y = $1%100 + 100 + 1900;
		$period = $y.$2.$3.'~'.$y.$5.$6;				
	} 
	return $period;
